...
describe("sample lang-python bundle", () => {
  it("manifest sha256 matches bundled wasm files", async () => {
    const root = join(process.cwd(), "sample", "bundles", "lang-python", "0.0.0");
    ...
    expect(manifest.name).toBe("lang-python");
  ...
  it("installs with requireSignature=true (official dev key)", async () => {
    ...
    const installed = await installBundle("lang-python", "0.0.0", {
      registry,
      cache: {
        async read(path) {
    ...
    expect(installed.rootPath).toBe("bundles/lang-python/0.0.0");
    expect(cache.has("bundles/lang-python/0.0.0/python.wasm")).toBe(true);
  });
});
